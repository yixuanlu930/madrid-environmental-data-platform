...
from src.pipeline import run_pipeline, parse_date
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle(self):
        ...
        if parsed.path == "/health":
            self._send_json(200, {"status": "ok", "service": "madrid-etl"})
            return
        ...
        if parsed.path == "/run":
            ...
            try:
                target_day = parse_date(date_value)
            except ValueError:
    ...
    def log_message(self, format, *args):
        # Keep default logs concise but visible in docker logs.
